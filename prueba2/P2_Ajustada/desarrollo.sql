...
DECLARE
    ...
    TYPE V_array_type IS variableRAY(6) of number;
   v_array V_array_type := V_array_type(2,4,5,7,9,25000);

    TYPE V_array_type2 IS variableRAY(5) of number;
   v_array_eval V_array_type2 := V_array_type2(4,6,8,10,12);
   
    TYPE V_array_type3 IS variableRAY(4) of number;
   v_array_cont V_array_type3 := V_array_type3(5,5,10,15);

   TYPE V_array_type4 IS variableRAY(8) of number;
...
-- padre profesional hijo asesorias, parametro hijo tipo_profesion
-- padre asesorias, hijo profesionales, parametro hijo tipo_profesion
DROP SEQUENCE SEQ_ID;
CREATE SEQUENCE SEQ_ID;
TRUNCATE TABLE DETALLE_ASIGNACION_MES; 
TRUNCATE TABLE RESUMEN_MES_PROFESION;
...
cursor cur_resumen(cod_prof number) is
 ...
 ' asig profesion '|| rec_asesoria.asig_profesion||' total '||rec_asesoria.total);
 */
open cur_resumen(rec_asesoria.cod_prof);
loop
FETCH cur_resumen INTO 
rec_resumen.cod_profesion;
...
exit when cur_resumen%notfound;
dbms_output.put_line(' cod_prof ' ||rec_resumen.cod_profesion);
end loop;
close cur_resumen;
